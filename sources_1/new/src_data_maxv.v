...
// -------------------------------------------------------------------
// 基于异步FIFO的200MHz数据源与125MHz以太网链路速率匹配
// -------------------------------------------------------------------

module src_data_maxv(
    input               sys_clk_p,               // 200mhz差分输入
    input               sys_clk_n,
    input               sys_rst,
    ...
    // --------SGMII接口-------
    input               sgmii_clk_n,             // 125mhz差分输入
    input               sgmii_clk_p,
    input               sgmii_rxn,                      
    input               sgmii_rxp,
    output              sgmii_txn,
    output              sgmii_txp,              
    output              eth_rst_n,               // 以太网物理模块复位

    input               send_key,                // 按键触发arp发送          SW_W
    input               udp_sender_key,          // 按键触发UDP发送    SW_N
    output              arp_led                  // arp解析指示led
    ...
    parameter  DATA_LENGTH     = 32'd524_288_000;         // 总发送数据量:500MB
    parameter  UDP_SEND_LENGTH = 16'd1472;                // UDP数据段长度
    ...
    reg         send_start_pusle;                               // 发送脉冲触发
    reg         wr_data_done;                                   // FIFO写入数据完成

    reg         tx_busy;                                        // UDP发送忙
    reg [15:0]  tx_byte_num;                                    // UDP报文数据段长度
    reg [7:0]   fifo_rst_cnt;                                   // 异步FIFO复位计数
    reg         sys_clk_cnt;
    reg         src_data_clk;
    ...
    always @(posedge sys_clk or posedge sys_rst) begin
        if(sys_rst) begin
            ...
            case(state)
                ...
                WRITE_DATA:begin              // 写入数据部分内容，不超过1500-20-8=1472 Bytes
                    if(byte_cnt < DATA_LENGTH) begin
                        if(!prog_full) begin                    // 未达到阈值时写入
    ...
    // ------------------------------   触发UDP发送状态机  ----------------------------
    // always块监听rd_data_count触发发送UDP报文
